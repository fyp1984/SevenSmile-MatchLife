-- RPC: Reset DB data (for admin/scripts)
...
-- Columns for change-detection and richer search
ALTER TABLE IF EXISTS public.matches ADD COLUMN IF NOT EXISTS raw_hash TEXT;

CREATE INDEX IF NOT EXISTS idx_matches_players_text ON public.matches(players_text);
CREATE INDEX IF NOT EXISTS idx_matches_raw_hash ON public.matches(raw_hash);

-- RPC: Upsert batch, update only if changed
