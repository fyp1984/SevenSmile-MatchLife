-- Backfill event dimensions from raw JSON for existing rows

WITH parsed AS (
  SELECT
    id,
    NULLIF((regexp_match(raw->>'fullName', '(\\d{1,2})\\s*岁'))[1], '')::int AS age_years,
    NULLIF(
      COALESCE(
        NULLIF(raw->>'itemName', ''),
        NULLIF((regexp_match(raw->>'fullName', '\\d{1,2}\\s*岁\\s*([^\\s\\[]+)'))[1], '')
      ),
      ''
    ) AS item_name
  FROM public.matches
...
SET
  ...
  event_key = CASE
    WHEN p.age_years IS NOT NULL AND p.item_name IS NOT NULL THEN p.age_years::text || '岁' || p.item_name
    ELSE p.item_name
  END
FROM parsed p
WHERE m.id = p.id
  AND (m.event_key IS NULL OR m.item_name IS NULL OR m.age_years IS NULL);

-- Update UPSERT condition so new dimension columns can be backfilled even when raw_hash is unchanged
...
BEGIN
  ...
  WITH input AS (
    ...
    WHERE
      ...
      OR public.matches.age_years IS DISTINCT FROM EXCLUDED.age_years
      OR public.matches.item_name IS DISTINCT FROM EXCLUDED.item_name
      OR public.matches.event_key IS DISTINCT FROM EXCLUDED.event_key
